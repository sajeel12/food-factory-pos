...
async function performSync() {
    try {
        ...
        if (unsyncedOrders.length > 0) {
            ...
            try {
                const response = await fetch('http://localhost:3000/orders/sync', {
                    method: 'POST',
                    headers,
        ...
        try {
            const response = await fetch('http://localhost:3000/sync/pull?lastSyncDate=1970-01-01', { headers });
            if (response.ok) {
        ...
        try {
            const response = await fetch('http://localhost:3000/settings', { headers });
            if (response.ok) {
        ...
        if (branchId && token) {
            try {
                const response = await fetch(`http://localhost:3000/riders?branchId=${branchId}`, { headers });
                if (response.ok) {
            ...
            // Sync Customers
            try {
                const response = await fetch(`http://localhost:3000/customers`, { headers });
                if (response.ok) {
            ...
            // Sync Categories
            try {
                const catUrl = branchId && branchId !== 'null' ? `http://localhost:3000/categories?branchId=${branchId}` : `http://localhost:3000/categories`;
